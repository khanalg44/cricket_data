...
def read_ball_by_ball(f, data_dir='./'):
    ...
    for i, inn in enumerate(inns):
        ...
        for delivery in data['innings'][i][inn]['deliveries']:
            ...
            runs_type={str(i)+'s':0 for i in range(7)}
...
def print_scorecard(f, data_dir='./'):
    df = read_ball_by_ball(f, data_dir)
    ...
    for inn in all_inns:
        ...
        for player in all_players:
            ...
            if (dfbat.shape[0]>0) or (player in all_bat_out) :  # second condition if batsman runout for 0(0)
                ...
                runs_type = [sum([d[str(i)+'s'] for d in dfbat['runs_type']]) for i in range(7) ]
                zeros, ones, twos, threes, fours, fives, sixes = runs_type
                
                win  = (winner == batting_team)
                toss = (toss_winner == batting_team)

                info_bat = [player, runs, balls, not_out, zeros, ones, twos, threes, fours, fives, sixes,
            ...
            if dfbwl.shape[0]>0:
                ...
                runs_type = [sum( [d[str(i)+'s'] for d in dfbwl['runs_type']] ) for i in range(7) ]
                zeros, ones, twos, threes, fours, fives, sixes = runs_type
                ...
                info_ball = [player, ov, maidens, total_runs_given, wkt_taken, inn, ov_dict,
                             zeros, ones, twos, threes, fours, fives, sixes, wds, nbs, 
    ...
    col_bal = ['bowler', 'O', 'M', 'R', 'W', 'innings', 'all_overs',
               '0s', '1s', '2s', '3s', '4s', '5s', '6s', 'wds', 'nbs',
               'date', 'match_id', 'Win', 'draw', 'Toss']
    df_bowl = pd.DataFrame(bowling_card, columns=col_bal)
